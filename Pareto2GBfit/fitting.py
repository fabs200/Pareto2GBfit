...
def Paretofit(x, b, x0, weights=np.array([1]), bootstraps=500, method='SLSQP', omit_missings=True,
    ...
    if ci and verbose:
        ...
        tbl.add_row(['p', '{:.3f}'.format(np.mean(p_fit_bs)), '{:.3f}'.format(np.std(p_fit_bs)),
                     ...
                     '{:.3f}'.format(np.mean(p_fit_bs)-np.std(p_fit_bs)*1.96),
                     '{:.3f}'.format(np.mean(p_fit_bs)+np.std(p_fit_bs)*1.96), k])
        print(tbl)
...
def IB1fit(x, b, x0, weights=np.array([1]), bootstraps=500, method='SLSQP', omit_missings=True,
    ...
    if ci and verbose:
        ...
        tbl.add_row(['q', '{:.3f}'.format(np.mean(q_fit_bs)), '{:.3f}'.format(np.std(q_fit_bs)),
                     ...
                     '{:.3f}'.format(np.mean(q_fit_bs)-np.std(q_fit_bs)*1.96),
                     '{:.3f}'.format(np.mean(q_fit_bs)+np.std(q_fit_bs)*1.96), k])
        print(tbl)
...
def GB1fit(x, b, x0, weights=np.array([1]), bootstraps=250, method='SLSQP', omit_missings=True,
    ...
    if ci and verbose:
        ...
        tbl.add_row(['a', '{:.3f}'.format(np.mean(a_fit_bs)), ('{:.3f}'.format(np.std(a_fit_bs))),
                     ...
                     '{:.3f}'.format(np.mean(a_fit_bs)-np.std(a_fit_bs)*1.96),
                     '{:.3f}'.format(np.mean(a_fit_bs)+np.std(a_fit_bs)*1.96), k])
        tbl.add_row(['p', '{:.3f}'.format(np.mean(p_fit_bs)), ('{:.3f}'.format(np.std(p_fit_bs))),
                     '{:.3f}'.format(p_zscore), '{:.3f}'.format(p_pval),
                     '{:.3f}'.format(np.mean(p_fit_bs)-np.std(p_fit_bs)*1.96),
                     '{:.3f}'.format(np.mean(p_fit_bs)+np.std(p_fit_bs)*1.96), k])
...
def GBfit(x, b, x0, weights=np.array([1]), bootstraps=250, method='SLSQP', omit_missings=True,
    ...
    if ci and verbose:
        ...
        tbl.add_row(['c', '{:.3f}'.format(np.mean(c_fit_bs)), '{:.3f}'.format(np.std(c_fit_bs)),
                     ...
                     '{:.3f}'.format(np.mean(c_fit_bs)-np.std(c_fit_bs)*1.96),
                     '{:.3f}'.format(np.mean(c_fit_bs)+np.std(c_fit_bs)*1.96), k])
        tbl.add_row(['p', '{:.3f}'.format(np.mean(p_fit_bs)), '{:.3f}'.format(np.std(p_fit_bs)),
                     '{:.3f}'.format(p_zscore), '{:.3f}'.format(p_pval),
                     '{:.3f}'.format(np.mean(p_fit_bs)-np.std(p_fit_bs)*1.96),
                     '{:.3f}'.format(np.mean(p_fit_bs)+np.std(p_fit_bs)*1.96), k])
...
def Paretobranchfit(x, b, x0=np.array([-.1,.1,1,-.1]), weights=np.array([1]), bootstraps=250, method='SLSQP',
    ...
    if rejection_criteria == "AIC":
        ...
        if IB1_aic_nested > IB1_aic:
            if GB1_aic_nested > GB1_aic:
                bestmodel, Pareto_marker = '--', 'IB1'
                if GB_aic_nested > GB_aic:
                    GB_bm, bestmodel, GB_marker, GB_remaining = 'GB', 'GB', 'XX', True
                    Pareto_marker = IB1_marker = GB1_marker = GB1_marker = '--'
                else:
                    GB1_bm, bestmodel, GB1_marker = 'GB1', 'GB1', 'XX'
